...
// ═══════════════════════════════════════════════════════════════════════════════
// 🔧 AUTO-REPAIR SYSTEM - Selbstheilung (každou hodinu +15 minut po healthCheck)
// ═══════════════════════════════════════════════════════════════════════════════

exports.autoRepairSystem = functions
  .region(REGION)
  .pubsub.schedule('15 * * * *')
  .timeZone('Europe/Prague')
  .onRun(async () => {
    ...
    try {
      console.log('🔧 Spouštím autoRepairSystem...');

      // REPAIR 1: Pending transakce >24h → archivovat
      try {
        const cutoff24h = new Date(Date.now() - 86400000);
        const usersSnap = await db.collection('users').get();
        let archivedCount = 0;
        ...
        for (const userDoc of usersSnap.docs) {
          ...
          if (!pendingSnap.empty) {
            ...
            for (const pendingDoc of pendingSnap.docs) {
              ...
              // Archivovat do archive kolekce
              batch.set(
                db.collection('users').doc(userDoc.id).collection('archivedTransactions').doc(pendingDoc.id),
                { ...data, archivedAt: admin.firestore.FieldValue.serverTimestamp(), reason: 'AUTO_REPAIR_24H' }
              );
        ...
        if (archivedCount > 0) {
          repairs.push({
            type: 'ARCHIVE_PENDING_24H',
            severity: 'info',
            count: archivedCount,
            message: `Archivováno ${archivedCount} starých pending transakcí (>24h)`,
          });
          console.log(`✓ Archivováno ${archivedCount} pending transakcí`);
        }
      } catch (e) {
        console.error('❌ Archive pending failed:', e);
        repairs.push({ type: 'ARCHIVE_PENDING_FAIL', severity: 'error', message: e.message });
      }

      // REPAIR 2: Recurring >48h bez generace → force regenerate nebo deactivate
      try {
        const cutoff48h = new Date(Date.now() - 172800000);
        const usersSnap = await db.collection('users').get();
        ...
        for (const userDoc of usersSnap.docs) {
          ...
          for (const recurDoc of recurSnap.docs) {
            const data = recurDoc.data();
            // Pokus force regenerovat
            if (data.recurrenceType === 'daily' || data.recurrenceType === 'weekly') {
              // Force regenerate recent (poslední 7 dní)
              await db.collection('users').doc(userDoc.id).collection('repeatingTransactions')
                .doc(recurDoc.id)
                .update({
              ...
              // Monthly/Yearly → deactivate (bezpečnější)
              await db.collection('users').doc(userDoc.id).collection('repeatingTransactions')
                .doc(recurDoc.id)
                .update({
                  isActive: false,
                  deactivatedAt: admin.firestore.FieldValue.serverTimestamp(),
                  deactivationReason: 'AUTO_REPAIR_48H_STALE',
                });
              deactivatedCount++;
            }
          }
        ...
        if (regeneratedCount > 0 || deactivatedCount > 0) {
          repairs.push({
            type: 'REPAIR_RECURRING',
            severity: 'info',
            count: regeneratedCount + deactivatedCount,
            message: `Regenerováno ${regeneratedCount} recurring, deaktivováno ${deactivatedCount} (>48h bez generace)`,
          });
          console.log(`✓ Regenerováno ${regeneratedCount}, deaktivováno ${deactivatedCount}`);
        }
      } catch (e) {
        console.error('❌ Repair recurring failed:', e);
        repairs.push({ type: 'REPAIR_RECURRING_FAIL', severity: 'error', message: e.message });
      }

      // REPAIR 3: Duplikáty → deduplicate
      try {
        const usersSnap = await db.collection('users').get();
        ...
        for (const userDoc of usersSnap.docs) {
          // Check vydaje
          const vydajeSnap = await db.collection('users').doc(userDoc.id).collection('vydaje').get();
          const vydajeDups = new Map();

          for (const doc of vydajeSnap.docs) {
            const data = doc.data();
            const key = `${data.date}_${data.amount}_${data.category}`;
            if (vydajeDups.has(key)) {
              await doc.ref.delete();
              deduplicatedCount++;
            } else {
              vydajeDups.set(key, doc.id);
            }
          }

          // Check příjmy
          const prijmySnap = await db.collection('users').doc(userDoc.id).collection('prijmy').get();
          const prijmyDups = new Map();

          for (const doc of prijmySnap.docs) {
            const data = doc.data();
            const key = `${data.date}_${data.amount}_${data.category}`;
            if (prijmyDups.has(key)) {
              await doc.ref.delete();
              deduplicatedCount++;
            } else {
              prijmyDups.set(key, doc.id);
            }
          }
        }
        ...
        if (deduplicatedCount > 0) {
          repairs.push({
            ...
            message: `Smazáno ${deduplicatedCount} duplikátních transakcí`,
          });
          console.log(`✓ Smazáno ${deduplicatedCount} duplikátů`);
        }
      } catch (e) {
        console.error('❌ Deduplication failed:', e);
        repairs.push({ type: 'DEDUPLICATE_FAIL', severity: 'error', message: e.message });
      }

      // REPAIR 4: Invalid/corrupted docs → cleanup
      try {
        const usersSnap = await db.collection('users').get();
        let cleanedCount = 0;
        ...
        for (const userDoc of usersSnap.docs) {
          ...
          // Check na required fields
          if (!userData.email || !userData.username || !userData.createdAt) {
            console.warn(`⚠️ Corrupted user doc: ${userDoc.id}`);
            // Neopravujeme user docs (příliš kritické), jen loggujeme
          }

          // Cleanup invalid vydaje/příjmy (missing amount, date, atd.)
          const vydajeSnap = await db.collection('users').doc(userDoc.id).collection('vydaje').get();
          for (const doc of vydajeSnap.docs) {
            const data = doc.data();
            if (!data.amount || !data.date || data.amount < 0) {
              await doc.ref.delete();
              cleanedCount++;
            }
          }
        ...
        if (cleanedCount > 0) {
          repairs.push({
            type: 'CLEANUP_INVALID',
            severity: 'warning',
            count: cleanedCount,
            message: `Vyčištěno ${cleanedCount} invalid dokumentů (negative amount, missing fields, atd.)`,
          });
          console.log(`✓ Vyčištěno ${cleanedCount} invalid docs`);
        }
      } catch (e) {
        console.error('❌ Cleanup invalid docs failed:', e);
        repairs.push({ type: 'CLEANUP_FAIL', severity: 'error', message: e.message });
      }

      // Uloží repair log do Firestore (jen když je něco na opravit)
      if (repairs.length > 0) {
        const repairLog = {
          timestamp: admin.firestore.FieldValue.serverTimestamp(),
      ...
      try {
        const failLog = {
          ...
          status: 'FAILED',
          error: err.message,
          repairs: [{ type: 'SYSTEM_ERROR', severity: 'error', message: err.message }],
        };
        const docId = new Date().toISOString();
        await db.collection('systemRepairs').doc(docId).set(failLog);
      } catch (logErr) {
